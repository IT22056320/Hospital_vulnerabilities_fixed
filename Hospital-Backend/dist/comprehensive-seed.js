...
const staff_model_1 = __importDefault(require("../src/models/staff.model"));
...
function seedDatabase() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield staff_model_1.default.deleteMany({});
            let createdCount = 0;
            for (const demoUser of demoUsers) {
                try {
                    console.log(`👤 Creating ${demoUser.role}: ${demoUser.name || demoUser.username}`);
                    // Hash the password
                    const hashedPassword = yield hashPassword(demoUser.password);
                    // Create User record
                    ...
                    if (demoUser.role !== 'PATIENT') {
                        if (demoUser.department)
                            staffData.department = demoUser.department;
                        if (demoUser.specialization)
                            staffData.specialization = demoUser.specialization;
                        if (demoUser.workExperience)
                            staffData.workExperience = demoUser.workExperience;
                        if (demoUser.degree)
                            staffData.degree = demoUser.degree;
                    }
                    if (demoUser.about)
                        staffData.about = demoUser.about;
                    const staff = yield staff_model_1.default.create(staffData);
